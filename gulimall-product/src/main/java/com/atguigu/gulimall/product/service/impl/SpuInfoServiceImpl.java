...
import com.atguigu.gulimall.product.fegin.CouponFeginService;
import com.atguigu.gulimall.product.fegin.SearchFeginService;
import com.atguigu.gulimall.product.fegin.WareFeginService;
...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void saveSpuInfo(SpuSaveVo vo) {
        ...
        //4.保存spu的规划参数;pms_product_attr_value
        List<BaseAttrs> baseAttrs = vo.getBaseAttrs();
        List<ProductAttrValueEntity> collect = baseAttrs.stream().map(attr -> {
            ProductAttrValueEntity valueEntity = new ProductAttrValueEntity();
            valueEntity.setId(attr.getAttrId());
        ...
        //5.保存spu的积分信息; gulimall_sms-> sms_spu_bounds
             Bounds bounds = vo.getBounds();
              ...
              spuBoudsTo.setSpuId(spuInfoEntity.getId());
        R r = couponFeginService.saveSpuBouds(spuBoudsTo);
        if(r.getCode()!=0){
    ...
    public void up(Long spuId) {
       ...
       List<SkuInfoEntity> skus  =  skuInfoService.getSkusBySpuId(spuId);
        ...
        List<ProductAttrValueEntity> baseAttrs = attrValueService.baseAttrListlistforspu(spuId);
        List<Long> attrIds = baseAttrs.stream().map(attr -> {
            return attr.getAttrId();
        }).collect(Collectors.toList());
        List<Long> searchAttrsIds  =attrService.selectSearchAttrsIds(attrIds);
...
//        try {
//            R<List<SkuHasStockVo>> skusHasStock = wareFeginService.getSkusHasStock(skuIdList);
//        stockMap = skusHasStock.getData().stream().collect(Collectors.toMap(SkuHasStockVo::getSkuId,item -> item.getHasStock()));
//        }catch (Exception e){
//            log.error("库存服务查询异常:原因{}",e);
//        }
        try {
            R skuHasStock = wareFeginService.getSkusHasStock(skuIdList);
            //
